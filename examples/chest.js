...
async function watchChest(minecart, blocks = []) {
    ...
    if (minecart) {
        chestToOpen = Object.keys(bot.entities)
            ...
            .find((e) => e.entityType === bot.registry.entitiesByName.chest_minecart && e.objectData.intField === 1 && bot.entity.position.distanceTo(e.position) < 3)
        if (!chestToOpen) {
    ...
    function onChat(username, message) {
        ...
        async function addLapis() {
            const item = itemByType(
                table.window.items(),
                ["dye", "purple_dye", "lapis_lazuli"].filter((name) => bot.registry.itemByName[name] !== undefined).map((name) => bot.registry.itemByName[name].id),
            )
            if (item) {
                try {
