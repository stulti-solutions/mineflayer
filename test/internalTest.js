...
for (const supportedVersion of mineflayer.testedVersions) {
    ...
    function chatText(text) {
        ...
        return registry.supportFeature("chatPacketsUseNbtComponents") ? nbt.comp({ text: nbt.string(text) }) : JSON.stringify({ text })
    }
    ...
    describe(`mineflayer_internal ${supportedVersion}v`, function () {
        ...
        describe("game", () => {
            it("responds to ping / transaction packets", (done) => {
                // only on 1.17
                server.on("playerJoin", async (client) => {
                ...
                server.on("playerJoin", (client) => {
                    bot.on("entitySpawn", (entity) => {
                        ...
                        client.write("entity_metadata", {
                            entityId: 8,
                            metadata: [{ key: 0, type: bot.registry.supportFeature("mcDataHasEntityMetadata") ? "int" : 0, value: 1 }],
                        })
                    })
            ...
            it("'itemDrop' event", function (done) {
                ...
                server.on("playerJoin", (client) => {
                    ...
                    const metadataPacket = {
                        entityId: 16,
                        metadata: [{ key: 7, type: 6, value: { itemCount: itemData.itemCount } }],
                    }
        ...
        it("bed", (done) => {
            ...
            const beds = [
                ...
                { head: vec3(9, 0, 4), foot: vec3(10, 0, 4), facing: 3, throws: true, error: new Error("the bed is too far") },
                {
                    ...
                    facing: 0,
                    throws: true,
                    error: new Error("there are monsters nearby"),
                },
