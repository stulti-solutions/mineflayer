...
for (const supportedVersion of mineflayer.testedVersions) {
	...
	describe(`mineflayer_internal ${supportedVersion}v`, function () {
		...
		it("entity effects", (done) => {
			...
			server.on("playerJoin", (client) => {
				client.write(
					bot.registry.supportFeature("consolidatedEntitySpawnPacket") ? "spawn_entity" : "spawn_entity_living",
					{
						entityId: 8, // random
			...
			it("'itemDrop' event", function (done) {
				...
				server.on("playerJoin", (client) => {
					...
					if (["1.8", "1.9", "1.10", "1.11", "1.12"].includes(bot.majorVersion)) {
						...
						entityType = bot.registry.entitiesArray.find(
							(e) => e.name.toLowerCase() === "item" || e.name.toLowerCase() === "item_stack",
						).id
					}
					client.write("spawn_entity", {
						entityId: 16,
