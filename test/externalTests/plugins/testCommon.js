...
function inject(bot) {
	...
	async function setCreativeMode(value) {
		...
		const msgProm = onceWithCleanup(bot, "message", {
			timeout,
			checkCondition: (msg) =>
				gameModeChangedMessages.includes(msg.translate) && i++ > 0 && bot.game.gameMode === getGM(value),
		})
	...
	async function teleport(position) {
		if (bot.supportFeature("hasExecuteCommand")) {
			bot.test.sayEverywhere(
				`/execute in overworld run teleport ${bot.username} ${position.x} ${position.y} ${position.z}`,
			)
		} else {
