...
module.exports = () => async (bot) => {
	...
	await assert.rejects(bot.consume, (err) => {
		if (!err) {
			...
			console.log({
				a: bot.game.gameMode !== "creative",
				b: !["potion", "milk_bucket", "enchanted_golden_apple", "golden_apple"].includes(bot.heldItem.name),
				c: bot.food === 20,
			})
		}
	...
	while (bot.food > 0) {
		...
		else if (bot.supportFeature("effectAreMinecraftPrefixed"))
			bot.test.sayEverywhere(`/effect ${bot.username} minecraft:hunger 1 255`)
