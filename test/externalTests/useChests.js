...
module.exports = () => async (bot) => {
	...
	const largeTrappedChestLocations = [new Vec3(-1, bot.test.groundY, 1), new Vec3(-1, bot.test.groundY, 0)]
	const chestSlot = 36
	...
	async function handler(block, isOpen, block2) {
		if (emitted) {
			...
			let blockAssert = false
			let block2Assert = false
			...
			assert(
				blockAssert && block2Assert,
				new Error("The block instance emitted by chestLidMove is not part of the chest oppened"),
			)
