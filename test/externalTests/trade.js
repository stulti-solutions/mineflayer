...
module.exports = () => async (bot) => {
	...
	function verifyTrade(trade) {
		...
		bot.test.sayEverywhere(
			`I have ${printCountInv(trade.inputItem1)} ${trade.hasItem2 ? "and " + printCountInv(trade.inputItem2) : ""}`,
		)
		bot.test.sayEverywhere(
			`I can trade ${printCountTrade(trade.inputItem1)} ${trade.hasItem2 ? "and " + printCountTrade(trade.inputItem2) : ""} for ${printCountTrade(trade.outputItem)}`,
		)
	}
