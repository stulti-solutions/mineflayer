...
const ALWAYS_CONSUMABLES = ["potion", "milk_bucket", "enchanted_golden_apple", "golden_apple"]
...
function inject(bot, { hideErrors }) {
    ...
    bot.on("spawn", () => {
        Object.defineProperty(bot.entity, "equipment", {
            get: bot.supportFeature("doesntHaveOffHandSlot")
                ? function () {
                      return [bot.heldItem, bot.inventory.slots[8], bot.inventory.slots[7], bot.inventory.slots[6], bot.inventory.slots[5]]
                  }
                : function () {
                      return [bot.heldItem, bot.inventory.slots[45], bot.inventory.slots[8], bot.inventory.slots[7], bot.inventory.slots[6], bot.inventory.slots[5]]
                  },
        })
    })
        ...
        async function transferOne() {
            ...
            async function clickDest() {
                ...
                if (destStart === -999) {
                    ...
                    destItem = window.findItemRange(destStart, destEnd, window.selectedItem.type, window.selectedItem.metadata, true, nbt)
    ...
    function tradeMatch(limitItem, targetItem) {
        return targetItem !== null && limitItem !== null && targetItem.type === limitItem.type && targetItem.count >= limitItem.count
    }
    ...
    bot._client.on("open_horse_window", (packet) => {
        // open window
        bot.currentWindow = windows.createWindow(packet.windowId, "HorseWindow", "Horse", packet.nbSlots)
        prepareWindow(bot.currentWindow)
    })
