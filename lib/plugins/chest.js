...
function inject(bot) {
	...
	async function openContainer(containerToOpen, direction, cursorPos) {
		...
		if (
			...
			allowedWindowTypes.map((name) => name.replace("minecraft:", "")).includes(containerToOpen.name)
		) {
		...
		if (!matchWindowType(chest)) {
			throw new Error("Non-container window used as a container: " + JSON.stringify(chest))
		}
		return chest
	}
