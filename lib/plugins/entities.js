...
function inject(bot) {
	...
	const hasFireworkSupport =
		fireworkEntityName !== undefined && fireworkMetadataIdx !== undefined && fireworkMetadataIsOpt !== undefined
	...
	bot._client.on("entity_metadata", (packet) => {
		...
		if (bot.supportFeature("mcDataHasEntityMetadata")) {
			...
			const typeSlot =
				(bot.supportFeature("itemsAreAlsoBlocks") ? 5 : 6) + (bot.supportFeature("entityMetadataHasLong") ? 1 : 0)
		...
		if (bot.supportFeature("playerInfoActionIsBitfield")) {
			for (const item of packet.data) {
				...
				if (item.displayName) {
					...
					obj.displayName = new ChatMessage({ text: "", extra: [{ text: player.username || obj.username }] })
