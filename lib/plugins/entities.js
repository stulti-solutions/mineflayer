...
function inject(bot) {
    ...
    bot._client.on("spawn_entity", (packet) => {
        ...
        const entity =
            entityData?.type === "player" ? addNewPlayer(packet.entityId, packet.objectUUID, packet) : addNewNonPlayer(packet.entityId, packet.objectUUID, packet.type, packet)
        bot.emit("entitySpawn", entity)
    })
