...
const SENSITIVITY = Math.random() * 0.1 + 0.05
...
function inject(bot, { physicsEnabled, maxCatchupTicks }) {
    ...
    function sendPacketPosition(position, onGround) {
        // sends data, no logic
        const oldPos = new Vec3(lastReported.x, lastReported.y, lastReported.z)
        lastReported.x = position.x
        lastReported.y = position.y
        lastReported.z = position.z
        lastReported.onGround = onGround
        bot._client.write("position", lastReported)
    ...
    function sendPacketLook(yaw, pitch, onGround) {
        // sends data, no logic
        const oldPos = new Vec3(lastReported.x, lastReported.y, lastReported.z)
        lastReported.yaw = yaw
        lastReported.pitch = pitch
        lastReported.onGround = onGround
        bot._client.write("look", lastReported)
        bot.emit("move", oldPos)
    }
    ...
    function sendPacketPositionAndLook(position, yaw, pitch, onGround) {
        ...
        lastReported.pitch = pitch
        lastReported.onGround = onGround
        bot._client.write("position_look", lastReported)
        bot.emit("move", oldPos)
    }
    ...
    function updatePosition(now) {
        ...
        const dYaw = deltaYaw(bot.entity.yaw, lastReportedYaw)
        const dPitch = bot.entity.pitch - (lastReportedPitch || 0)
        ...
        lastReportedYaw += math.clamp(-maxDeltaYaw, dYaw, maxDeltaYaw)
        lastReportedPitch += math.clamp(-maxDeltaPitch, dPitch, maxDeltaPitch)

        const yaw = Math.fround(conv.toNotchianYaw(lastReportedYaw))
        const pitch = Math.fround(conv.toNotchianPitch(lastReportedPitch))
        const position = bot.entity.position
        const onGround = bot.entity.onGround

        // Only send a position update if necessary, select the appropriate packet
        const positionUpdated = lastReported.x !== position.x || lastReported.y !== position.y || lastReported.z !== position.z || ticksSinceLastPosition >= 20
        const lookUpdated = lastReported.yaw !== yaw || lastReported.pitch !== pitch

        if (lastReported.sprinting !== controlState.sprint) {
            bot._client.write("entity_action", {
                entityId: bot.entity.id,
                actionId: controlState.sprint ? 3 : 4,
                jumpBoost: 0,
            })
            lastReported.sprinting = controlState.sprint
        }

        if (lastReported.sneaking !== controlState.sneak) {
            bot._client.write("entity_action", {
                entityId: bot.entity.id,
                actionId: controlState.sneak ? 0 : 1,
                jumpBoost: 0,
            })
            lastReported.sneaking = controlState.sneak
        }
        ...
        } else if (positionUpdateSentEveryTick || onGround !== lastReported.onGround) {
        ...
        lastReported.onGround = bot.entity.onGround // onGround is always set
    }
    ...
    bot.on("move", () => {
        if (!lookingTask.done && Math.abs(deltaYaw(bot.entity.yaw, lastReportedYaw)) < 0.001) {
            lookingTask.finish()
        }
    })
    ...
    bot.look = async (yaw, pitch, force) => {
        ...
        const sensitivity = conv.fromNotchianPitch(SENSITIVITY)
    ...
    bot._client.on("position", (packet) => {
        ...
        vel.set(packet.flags & 1 ? vel.x : 0, packet.flags & 2 ? vel.y : 0, packet.flags & 4 ? vel.z : 0)

        // If flag is set, then the corresponding value is relative, else it is absolute
        const pos = bot.entity.position
        pos.set(packet.flags & 1 ? pos.x + packet.x : packet.x, packet.flags & 2 ? pos.y + packet.y : packet.y, packet.flags & 4 ? pos.z + packet.z : packet.z)
        ...
        bot.jumpTicks = 0
        lastReportedYaw = bot.entity.yaw
        lastReportedPitch = bot.entity.pitch
